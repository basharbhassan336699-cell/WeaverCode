...
# OAuth access token — WeaverCode passes this for /login users.
...
_HAS_3P_PROVIDER_AT_LOAD = any(
    ...
    for v in (
        "WEAVER_CODE_USE_BEDROCK",
        "WEAVER_CODE_USE_VERTEX",
        "WEAVER_CODE_USE_FOUNDRY",
        "WEAVER_CODE_USE_MANTLE",
        "WEAVER_CODE_USE_ANTHROPIC_AWS",
    )
)
...
WEAVER_CODE_SYSTEM_PROMPT = "You are a Claude agent, built on Anthropic's Claude Agent SDK."
...
def ensure_anthropic_reachable() -> bool:
    ...
    WEAVER_CODE_REMOTE so local installs never pay the probe cost."""
    global _anthropic_reachable
    if _anthropic_reachable is not None:
        return _anthropic_reachable
    if os.environ.get("WEAVER_CODE_REMOTE", "").lower() not in ("1", "true", "yes", "on"):
...
# Mirrors WeaverCode's adaptive-thinking model support; keep in sync
...
def _call_claude_via_sdk(prompt, output_schema, *, max_tokens=16000, model=None):
    ...
    async def _arun():
        opts = ClaudeAgentOptions(
            system_prompt=WEAVER_CODE_SYSTEM_PROMPT,
            cli_path=cli_path,
            allowed_tools=[],
            setting_sources=[],
            max_turns=2,
...
def _call_claude(prompt, output_schema, thinking_budget=10000, max_tokens=16000, model=None,
    ...
    payload = {
        ...
        "system": WEAVER_CODE_SYSTEM_PROMPT,
...
**Agent/Subprocess Permission Bypass**: Code that spawns WeaverCode, a subagent, or any LLM-with-tools subprocess with permission gates removed — `--permission-mode bypassPermissions`, `--dangerously-skip-permissions`, or an unrestricted Bash/shell tool. Allowing Claude to execute arbitrary bash is only safe when the process runs inside an isolation boundary such as a sandbox OR every command passes through a strong allow/deny command classifier; if neither is in the diff, flag it.
...
def _agentic_spawn_env() -> Dict[str, str]:
    ...
    env = {
        ...
        "WEAVER_CODE_WEBSOCKET_AUTH_FILE_DESCRIPTOR": "",
        "WEAVER_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR": "",
        "WEAVER_CODE_INCLUDE_PARTIAL_MESSAGES": "",
...
def agentic_review(
    ...
    #
    # WEAVER_CODE_EXECPATH is the absolute path to the running CC binary
    ...
    if cli_path is None:
        for p in (
            os.environ.get("WEAVER_CODE_EXECPATH"),
            os.path.expanduser("~/.local/bin/claude"),
            "/root/.local/bin/claude",
            # WeaverCode Remote container install path. WEAVER_CODE_EXECPATH
    ...
    async def _arun(system: str, prompt: str, *, schema: Dict[str, Any],
        ...
        opts = ClaudeAgentOptions(
            ...
            # OAuth token) injected by WeaverCode. The SDK builds the child
