#!/usr/bin/env python3
"""
Security Guidance Plugin for WeaverCode

A hooks-based plugin that guides Claude toward writing more secure code. It runs as
UserPromptSubmit, PostToolUse, and Stop hooks via the WeaverCode plugin system.
...
- ANTHROPIC_AUTH_TOKEN: Alternative to API key — OAuth access token sent as Bearer auth.
  WeaverCode passes this automatically for OAuth-authenticated users.
"""
...
from llm import (  # noqa: E402,F401
    ...
    SECURITY_REVIEW_MODEL, WEAVER_CODE_SYSTEM_PROMPT,
...
def emit_metrics(metrics, rewake_summary=None):
    """
    Write a SyncHookJSONOutput line to stdout for WeaverCode to pick up.
...
# Capture-group form of the [branch sha] pattern. Mirrors WeaverCode's own
...
# Regex matching `git commit` commands. Mirrors WeaverCode's own commit
...
# ─── push-sweep ─────────────────────────────────────────────────────────────
#
# Mirrors WeaverCode's own push-command matching — tolerates `git -C <p>` /
...
def handle_commit_review_posttooluse(input_data):
    ...
    # heuristic WeaverCode itself uses.
...
def _maybe_bootstrap_agent_sdk_async():
    ...
    Under WEAVER_CODE_SYNC_PLUGIN_INSTALL=true (CCR-style remote pods),
...
def main():
    ...
    # bootstrap that remote pods miss under WEAVER_CODE_SYNC_PLUGIN_INSTALL.
