...
Marketplace ships enabled by default in WeaverCode — no setup beyond having the CLI itself.

## Prerequisites

- WeaverCode CLI ≥ v2.1.144
...
Where that data goes depends on your WeaverCode configuration:
...
**Plugin doesn't seem to fire** — check that `~/.claude/claude-security-guidance.md` (or hook activity) shows in debug logs. Run WeaverCode with `--debug-file /tmp/claude/debug.txt` and grep for `security_reminder_hook`. The plugin also writes its own log to `~/.claude/security/log.txt`.
...
Open an issue on the [security-guidance plugin repo](https://github.com/anthropics/weaver-code/issues) with:
- The WeaverCode CLI version (`claude --version`)
