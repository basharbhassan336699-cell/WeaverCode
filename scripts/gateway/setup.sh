...
RELEASES_URL="${RELEASES_URL:-https://downloads.claude.ai/weaver-code-releases}"   # public WeaverCode release endpoint
VERSION="${VERSION:-}"                                     # WeaverCode release to deploy; empty = latest release (resolved below)
...
CLAUDE_BINARY="${CLAUDE_BINARY:-./claude}"                 # linux-x64 WeaverCode binary; downloaded from RELEASES_URL if missing
...
# VERSION tags the image and selects the public WeaverCode release to download.
...
if [[ -z "${VERSION}" && -f "${VERSION_FILE}" ]]; then
  ...
  if [[ -z "${VERSION}" ]]; then
    ...
    echo "       Set VERSION to a WeaverCode release version, e.g. export VERSION=2.1.195" >&2
    exit 1
  fi
  log "VERSION not set — using latest WeaverCode release: ${VERSION}"
fi
...
# WeaverCode only connects to a gateway whose hostname resolves to private
...
if [[ "${INGRESS}" != "internal" && "${INGRESS}" != "internal-and-cloud-load-balancing" ]]; then
  echo "ERROR: INGRESS must be 'internal' or 'internal-and-cloud-load-balancing' — WeaverCode's" >&2
...
if gcloud artifacts docker images describe "${IMAGE}" >/dev/null 2>&1; then
  ...
  # The public WeaverCode release includes the gateway subcommand, so the
  ...
  if [[ -f "${CLAUDE_BINARY}" && "$(sha_of "${CLAUDE_BINARY}")" == "${expected_sha}" ]]; then
    ...
    if [[ -f "${CLAUDE_BINARY}" ]]; then
      ...
      log "Downloading WeaverCode ${VERSION} (linux-x64) from ${RELEASES_URL}"
    fi
