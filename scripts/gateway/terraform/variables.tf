...
variable "image_tag" {
  description = "Image tag — the WeaverCode release version you build and push (must already be pushed as linux/amd64). See the README Deploy section for the build command."
  type        = string
  validation {
    condition     = can(regex("^[A-Za-z0-9_][A-Za-z0-9._-]{0,127}$", var.image_tag))
    error_message = "image_tag must be a valid OCI tag — set it to the WeaverCode release version you pushed (the '<version>' in terraform.tfvars.example is a placeholder)."
  }
}
