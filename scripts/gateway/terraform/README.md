...
# 2. Download the public WeaverCode linux-x64 release binary (it includes the
...
BASE="https://downloads.claude.ai/weaver-code-releases"
...
- `ingress` — defaults to **internal-only** (no public URL). WeaverCode's `/login`
...
and there is no public-ingress option: WeaverCode's `/login` rejects gateway hosts that
