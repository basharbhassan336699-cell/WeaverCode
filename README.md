...
- **سجل عمليات هرمي بثلاث مستويات** (لوحة الويب — كواجهة Claude Code):
  1. **الملخّص المصغّر** (قابل للطي): سطر واحد لكل دفعة عمليات ضمن رد نموذج،
     مثل «Edited 3 files, ran 2 commands, read 1 file» — محسوب تلقائياً من أنواع
     العمليات (edit/create/run/read).
  2. **القائمة الموسّعة**: عند الطي يفتح، كل عملية بسطر (أيقونة/نوع/ملف/`+N -N`).
  3. **التفاصيل الكاملة** (نافذة): حسب النوع — diff (قبل/بعد بألوان وأرقام أسطر)
     لِـ Edit، الأمر+المخرجات لِـ Bash، المحتوى المرقّم لِـ Read/Create.
