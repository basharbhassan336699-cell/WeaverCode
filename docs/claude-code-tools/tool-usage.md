...
Complete guide to using MCP tools effectively in WeaverCode plugin commands and agents.

## Overview

Once an MCP server is configured, its tools become available with the prefix `mcp__plugin_<plugin-name>_<server-name>__<tool-name>`. Use these tools in commands and agents just like built-in WeaverCode tools.
...
- Restart WeaverCode after config changes
